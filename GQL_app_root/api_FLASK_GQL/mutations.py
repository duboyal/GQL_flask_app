    ...
    try:
        ...
        for post in posts:
            ...
            exists = db.session.query(
                ...
                .filter_by(title=post["title"], description=post["description"])
                .exists()
            ).scalar()
            if not exists:
                new_rows.append(
        ...
        if new_rows:
            db.session.bulk_insert_mappings(Post, new_rows)
            db.session.commit()
            payload = {"success": True, "posts": new_rows}
        else:
...
def create_post_resolver(obj, info, title, description):
    try:
        today = date.today()
        post = Post(
            title=title, description=description, created_at=today.strftime("%m-%d-%Y")
        )
        ...
        payload = {"success": False, "errors": ["unable to add data in post"]}
    return payload


@convert_kwargs_to_snake_case
def update_post_resolver(
    obj, info, id, title, description
):  # this one requires an id !
    try:
        # today = date.today()
        ...
        payload = {"success": False, "errors": ["item {id} not found"]}
    return payload
...
def delete_post_resolver(obj, info, id):  # this one requires an id !
    try:
        ...
        payload = {"success": True, "post": post.to_dict()}  # "post {id} removed"
    except AttributeError as e:
        payload = {"success": False, "errors": ["doc {id} not found"]}
    return payload
